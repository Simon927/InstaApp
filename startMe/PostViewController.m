...
//                    NSString *post_id = [Utility encryptString:comment_id];
//                    
//                    NSString *str = [URL_SERVER stringByAppendingString:@"request.php"];
//                    
//                    ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:[NSURL URLWithString:str]];
...
- (void)shareOnFacebook {
    ...
    if([_post.user_id isEqualToString:my_id] && !POST_HAVE_MEDIA) {
        graphPath = @"me/photos";
        
        [params setObject:_post.descr forKey:@"message"];
        
        if (POST_HAVE_IMAGE) {
            [params setObject:UIImageJPEGRepresentation(_post.image,1.0) forKey:@"source"];
        }
        /*
         else if(POST_HAVE_MEDIA) {
        ...
        [params setObject:_post.title forKey:@"name"];
        
        [params setObject:[NSString stringWithFormat:@"%@ %@",_post.name,_post.surname] forKey:@"caption"];
        [params setObject:_post.descr forKey:@"description"];
        
        if (POST_HAVE_IMAGE) {
            NSString *linkImage = [[URL_SERVER stringByAppendingString:PATH_IMAGES_POSTS] stringByAppendingString:_post.strImage];
            [params setObject:@"picture" forKey:@"type"];
            [params setObject:linkImage forKey:@"picture"];
        } else if(POST_HAVE_MEDIA) {
            
            NSString *link = _post.mediaLink;
