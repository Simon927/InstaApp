...
#define TAG_BUTTON_SHARE 28

#define TAG_BUTTON_DELETE 90
#define TAG_BUTTON_REPORT 88
...
- (void)viewDidLoad
{
    ...
    if(hashtag) {
        ...
        _id_from = (self.tabBarController.selectedIndex == 0) ? TYPE_POSTS_FOLLOWED : TYPE_POSTS_ALL;
        
        /***Localized string nib***/
        [self.navigationItem setTitle:([_id_from isEqualToString:TYPE_POSTS_FOLLOWED]) ? @"" : NSLocalizedString(@"titleFollowing", nil)];
        /********/
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (IS_SEARCHING) {
        ...
        if(indexPath.row == 0) {
            ...
            UIButton *btReport = (UIButton *)[cell viewWithTag:TAG_BUTTON_REPORT];
            [btReport setTitle:NSLocalizedString(@"btReport", nil) forState:UIControlStateNormal];
            [btReport addTarget:self action:@selector(touchMyButton:event:) forControlEvents:UIControlEventTouchUpInside];
            [btReport setHidden:!btDelete.hidden];
            ...
            UIButton *btShare = (UIButton *)[cell viewWithTag:TAG_BUTTON_SHARE];
            [btShare addTarget:self action:@selector(touchMyButton:event:) forControlEvents:UIControlEventTouchUpInside];
            [btShare setEnabled:(POST_HAVE_IMAGE || POST_HAVE_MEDIA)];
        }
    ...
    if (request.responseStatusCode == 200) {
        ...
        if([logged isEqualToString:@"1"]) {
            ...
            if([response isEqualToString:@"-1"]) {
                ...
                if ([request.method isEqualToString:SERVICE_REPORT_POST]) {
                    ...
                    HUD.delegate = self;
                    HUD.labelText = NSLocalizedString(@"Report sent!", nil);
...
- (void)touchMyButton:(UIButton*)button event:(UIEvent*)event
{
    ...
    switch (button.tag) {
        ...
        case TAG_BUTTON_SHARE: {
            ...
            UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:NSLocalizedString(@"Choose sharing", nil)
                                                               delegate:self
                                                      cancelButtonTitle:NSLocalizedString(@"Cancel", nil)
                                                 destructiveButtonTitle:nil
                                                      otherButtonTitles:@"Facebook", nil];//@"Twitter", nil];
//            [sheet showInView:self.view];
            [sheet showFromTabBar:self.tabBarController.tabBar];
            
            break;
        }
        case TAG_BUTTON_REPORT: {
            ...
            MGActionSheet *sheet = [[MGActionSheet alloc] initWithTitle:NSLocalizedString(@"Action report", nil)
                                                      cancelButtonTitle:NSLocalizedString(@"Cancel", nil)
                                                 destructiveButtonTitle:nil
                                                      otherButtonTitles:NSLocalizedString(@"btReport", nil), nil];

            [sheet showFromTabBar:self.tabBarController.tabBar withChoiceCompletition:^(int buttonIndex) {
            
//            [sheet showInView:self.view withChoiceCompletition:^(int buttonIndex) {
                if(buttonIndex != sheet.cancelButtonIndex) {
                    ...
                    NSString *session = [Utility encryptString:[Utility getSession]];
                    NSString *token = [Utility encryptString:[Utility getDeviceToken]];
                    NSString *method = [Utility encryptString:SERVICE_REPORT_POST];
                    NSString *post_id = [Utility encryptString:post.post_id];
                    ...
                    [request setPostValue:method forKey:@"method"];
                    [request setPostValue:post_id forKey:@"post_id"];
                    [request setMethod:SERVICE_REPORT_POST];
                    [request setDelegate:self];
                    [arrRequest addObject:request];
                    [request startAsynchronous];
                }
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex != [actionSheet cancelButtonIndex]) {
        switch (buttonIndex) {
                //Facebook button
            case 0: {
                [self startModeLoadingWithText:NSLocalizedString(@"Sharing on facebook", nil)];
                
                NSArray *write_permissions = [[NSArray alloc] initWithObjects:@"publish_stream", nil];
                
                if (!FBSession.activeSession.isOpen) {
                    [FBSession openActiveSessionWithPublishPermissions:write_permissions defaultAudience:FBSessionDefaultAudienceEveryone allowLoginUI:YES completionHandler:
                     ^(FBSession *session,
                    ...
                    if([[[FBSession activeSession] permissions] indexOfObject:@"publish_stream"] == NSNotFound) {
                        [[FBSession activeSession] reauthorizeWithPublishPermissions:write_permissions defaultAudience:FBSessionDefaultAudienceEveryone completionHandler:
...
- (void)shareOnFacebook {
    ...
    Post *post = (Post *)[arrPosts objectAtIndex:selIndex.section-offsetCellPost];
    
    NSString *graphPath = nil;
    
    //If is myself, publish a photo, otherwise a link with photo etc.
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    
    if([post.user_id isEqualToString:my_id] && !POST_HAVE_MEDIA) {
        graphPath = @"me/photos";
        
        [params setObject:post.descr forKey:@"message"];
        
        if (POST_HAVE_IMAGE) {
            [params setObject:UIImageJPEGRepresentation(post.image,1.0) forKey:@"source"];
        }
        /*
         else if(POST_HAVE_MEDIA) {
         [params setObject:@"link" forKey:@"type"];
         [params setObject:post.mediaLink forKey:@"link"];
         }
         */
    } else {
        graphPath = @"me/feed";
        
        [params setObject:post.title forKey:@"name"];
        
        [params setObject:[NSString stringWithFormat:@"%@ %@",post.name,post.surname] forKey:@"caption"];
        [params setObject:post.descr forKey:@"description"];
        
        if (POST_HAVE_IMAGE) {
            NSString *linkImage = [[URL_SERVER stringByAppendingString:PATH_IMAGES_POSTS] stringByAppendingString:post.strImage];
            [params setObject:@"picture" forKey:@"type"];
            [params setObject:linkImage forKey:@"picture"];
        } else if(POST_HAVE_MEDIA) {
            
            NSString *link = post.mediaLink;
            
            //If youtube (now ever yes) {
            link = [link stringByReplacingOccurrencesOfString:@"/v/" withString:@"/watch/?v="];
            NSString *thumb = [NSString stringWithFormat:@"http://img.youtube.com/vi/%@/default.jpg",[Utility getYoutubeVideoID:link]];
            [params setObject:thumb forKey:@"picture"];
            //}
            
            [params setObject:@"link" forKey:@"type"];
            [params setObject:link forKey:@"video"];
        }
    }
    
    
    //id<FBGraphObject> obj = (FBGraphObject*)params;
    ...
    [FBRequestConnection startWithGraphPath:graphPath
                          ...
                          completionHandler:^(FBRequestConnection *connection, id result, NSError *error) {
                              ...
                              if(error) {
                                  ...
                                  HUD.delegate = self;
                                  HUD.labelText = NSLocalizedString(@"Shared with success!", nil);
